...
単色・元素色はパレットを一度解決してまとめて設定し、座標や結合の Python 側へのコピーを省きます。既定の複合プリセットはポリマーだけから独自メッシュを作り、選択した脂質・溶媒・イオンは標準原子/結合表示で保持します。`color=keep` は元の色インデックスを保ち、独自形状では色ごとに一度だけ RGB を取得します。新たな品質設定や原子除外ルールはありません。
...
原子数・状態数・準備時間・プロセス最大メモリを記録します。構造ファイルと生成結果はバージョン管理しません。
